...
class AvailableProjectPluginTestCase(FuglViewTestCase):

    url = '/project_plugins/available/'
    ...
    def setUp(self):
        ...
        self.project = self.create_project('project1', '',
            owner=self.admin_user)
        self.plug = self.create_project_plugin('plug', project=self.project)
    ...
    def test_view_access(self):
        access = self.create_access(self.admin_user, self.other_project,
            can_edit=False)
        plug = self.create_project_plugin('other-plug',
            project=self.other_project)
