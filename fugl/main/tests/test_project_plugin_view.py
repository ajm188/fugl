...
class CreateProjectPluginViewTestCase(FuglTestCase):
    ...
    def url_for(self, project):
        ...
        return '/project/%s/%s/project_plugin/new' % (project.owner.username,
                                                      project.title)
    ...
    def test_post_owner(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Project Plugin created!', response.content)
        plugs = ProjectPlugin.objects.filter(project=self.project,
                                             title=data['title'])
    ...
    def test_create_existing_plugin(self):
        ...
        plug = ProjectPlugin.objects.create(title=data['title'],
                                            markup=data['markup'],
    ...
    def test_get_invalid_user(self):
        url = '/project/idontexist/%s/project_plugin/new' % self.project.title
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_post_invalid_user(self):
        url = '/project/idontexist/%s/project_plugin/new' % self.project.title
        data = {'title': 'Serious Business', 'markup': 'whatever'}
        self.login()
        response = self.client.post(url, data)
        self.assertEqual(response.status_code, 404)

    def test_get_invalid_project(self):
        url = '/project/%s/doesntexist/project_plugin/new' % self.admin_user.username
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
...
class DeleteProjectPluginViewTestCase(FuglTestCase):
    def setUp(self):
        ...
        self.plugin = ProjectPlugin.objects.create(title='default',
                                                   markup='LOL',
                                                   project=self.project)
        self.plugin.save()
    ...
    def url_for(self, project, plugin):
        ...
        return '/project/%s/%s/project_plugin/delete/%s' % (project.owner.username,
                                                            project.title,
                                                            plugin.id)
    ...
    def test_post_owner(self):
        ...
        with self.assertRaises(ProjectPlugin.DoesNotExist):
            self.plugin.refresh_from_db()

    def test_get_invalid_user(self):
        url = '/project/idontexist/%s/project_plugin/delete/%s' % (self.project.title,
                                                                   self.plugin.id)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_invalid_project(self):
        url = '/project/%s/doesntexist/project_plugin/delete/%s' % (self.admin_user.username,
                                                                    self.plugin.id)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_invalid_plugin(self):
        url = '/project/%s/%s/project_plugin/delete/5464646' % (self.admin_user.username,
                                                                self.project.title)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
...
class UpdateProjectPluginViewTestCase(FuglTestCase):
    ...
    def url_for(self, project, plugin):
        # return the "delete plugin" url for a plugin
        return '/project/%s/%s/project_plugin/edit/%s' % (project.owner.username,
                                                          project.title,
                                                          plugin.id)
    ...
    def test_get_owner(self):
        ...
        self.assertIn('Project Plugin', content)
        self.assertIn(self.plugin.title, content)
    ...
    def test_get_invalid_user(self):
        url = '/project/idontexist/%s/project_plugin/edit/%s' % (self.project.title,
                                                                 self.plugin.id)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_invalid_project(self):
        url = '/project/%s/doesntexist/project_plugin/edit/%s' % (self.admin_user.username,
                                                                  self.plugin.id)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
    ...
    def test_get_invalid_category(self):
        url = '/project/%s/%s/project_plugin/edit/5464646' % (self.admin_user.username,
                                                              self.project.title)
        self.login()
        response = self.client.get(url)
        self.assertEqual(response.status_code, 404)
